...
potential2 = kubo.potential2o(dims, sample)*dV
...
result = np.zeros([NT, 1+conv], dtype=complex)
result[:,0] = tlist
for pos in [vel1, vel2]:
    # print(f"{pos=}",end=" ")
    print("pos",pos)
    n,m,o = pos
    result[:,1] += kubo.KPM_exact(SCALE,pos,mu,packR,tlist, potential2, Ncheb, NTcheb, vel1, vel2, primitives, orb_pos, flags)
    result[:,2] += kubo.KPM_exact(SCALE,pos,mu,packR,tlist, potential2, Ncheb//2, NTcheb, vel1, vel2, primitives, orb_pos, flags)
    result[:,3] += kubo.KPM_exact(SCALE,pos,mu,packR,tlist, potential2, Ncheb, NTcheb//2, vel1, vel2, primitives, orb_pos, flags)
print("")

np.savetxt(f"kubotime_graphene_nanoribbon_Ly{Ly}_Lx{Lx}_sample{sample}_W{W:2.2f}_seed{seed}_mu{mu:2.2f}_dV{dV:5.5f}.dat", np.real(result))
